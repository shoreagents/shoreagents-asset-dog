...
import { useState, useEffect, useMemo, useRef, useTransition, useCallback, memo } from 'react'
...
export function AssetsPageContent({ initialData }: AssetsClientProps) {
  const queryClient = useQueryClient()
  const router = useRouter()
  ...
  // Server renders skeleton, client must match on initial render
  ...
  const { data, isLoading, isFetching, error } = useQuery({
    ...
    initialData: initialData, // Use server-fetched data for initial render
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mt-6 ">
        ...
        <Card className="flex flex-col py-0 gap-2">
          ...
          <CardContent className="flex flex-col grow justify-center p-4 pt-0">
            {isMounted && !permissionsLoading && !isLoadingSummary ? (
              <div>
        ...
        <CardContent className="p-0 relative">
          ...
          {isFetching && data && (
            <div className="absolute inset-x-0 top-[83px] bottom-24 bg-background/50 backdrop-blur-sm z-20 flex items-center justify-center">
              <Spinner variant="default" size={24} className="text-muted-foreground" />
            </div>
          )}
          
          {/* Table Section - with loading/error states */}
          {!isMounted || permissionsLoading || (isLoading && !data) ? (
...
// Export AssetsPageContent as AssetsClient for use in the server component
export { AssetsPageContent as AssetsClient }
